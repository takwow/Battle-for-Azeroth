...
void AccountAchievementMgr::SendAchievementEarned(AchievementEntry const* achievement) const
{
    ...
    if (!(achievement->Flags & ACHIEVEMENT_FLAG_TRACKING_FLAG))
    {
        ...
        {
            // broadcast realm first reached
            WorldPackets::Achievement::BroadcastAchievement broadcastAchievement;
            broadcastAchievement.Name = _owner->GetPlayer()->GetName();
            broadcastAchievement.PlayerGUID = _owner->GetPlayer()->GetGUID();
            broadcastAchievement.AchievementID = achievement->ID;
            sWorld->SendGlobalMessage(broadcastAchievement.Write());
        }
        // if player is in world he can tell his friends about new achievement
