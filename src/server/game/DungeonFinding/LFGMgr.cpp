...
void LFGMgr::JoinLfg(Player* player, uint8 roles, LfgDungeonSet& dungeons)
{
    ...
    else                                                   // Add player to queue
    {
        ...
        {
            player->GetSession()->SendLfgUpdateStatus(LfgUpdateData(LFG_UPDATETYPE_JOIN_RAID_BROWSER, dungeons), false);
            SetState(guid, LFG_STATE_RAID_BROWSER);
        }
...
void LFGMgr::LeaveLfg(ObjectGuid guid, bool disconnected)
{
    ...
    switch (state)
    {
        ...
        case LFG_STATE_RAID_BROWSER:
        {
            lfg::LfgUpdateType type = state == LFG_STATE_QUEUED ?
                LFG_UPDATETYPE_REMOVED_FROM_QUEUE : LFG_UPDATETYPE_LEAVE_RAID_BROWSER;
            if (!gguid.IsEmpty())
            {
                LFGQueue& queue = GetQueue(gguid);
                queue.RemoveFromQueue(gguid);
