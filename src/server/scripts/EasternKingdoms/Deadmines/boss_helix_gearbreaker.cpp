        ...
        class spell_helix_sticky_bomb_periodic_trigger_AuraScript : public AuraScript
        {
            ...
            void HandleTick(AuraEffect const* aurEff)
            {
                ...
                if (Unit* caster = GetCaster())
                    if (SpellInfo const* spell = sSpellMgr->GetSpellInfo(GetSpellInfo()->GetEffect(EFFECT_0)->TriggerSpell))
                        if (Player* player = caster->SelectNearestPlayer(spell->GetEffect(EFFECT_0)->CalcRadius()))
                            if (!player->GetVehicleBase())
                                caster->CastSpell(GetCaster()->GetVictim(), GetSpellInfo()->GetEffect(EFFECT_0)->TriggerSpell, true);
            }
            ...
            void HandleHit(SpellEffIndex effIndex)
            {
                if (Unit* caster = GetCaster())
                    if (Unit* target = GetHitUnit())
                        ...
                        {
                            target->CastStop();
                            target->CastSpell(caster, GetSpellInfo()->GetEffect(effIndex)->BasePoints, false);
                        }
            }
        ...
        class spell_helix_oaf_smash_SpellScript : public SpellScript
        {
            ...
            void HandleLaunch(SpellEffIndex effIndex)
            {
                ...
                if (Unit* caster = GetCaster())
                {
                    ...
                    float speedxy = float(GetSpellInfo()->GetEffect(effIndex)->MiscValue) * 0.1;
                    float speedz = float(GetSpellInfo()->GetEffect(effIndex)->MiscValueB) * 0.1;
