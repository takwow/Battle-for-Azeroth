...
class go_deadmines_defias_cannon : public GameObjectScript
{
    public:
        ...
        struct go_deadmines_defias_cannonAI : public GameObjectAI
        {
            ...
            {
                if (GameObject* cannon = go->ToGameObject())
                {
                    if (_instance->GetData(DATA_BROKEN_DOOR) != DONE)
                    {
                        ...
                        if (Creature* bunny = go->FindNearestCreature(NPC_GENERAL_PURPOSE_DUMMY_JMF, 20.0f, true))
        ...
        class spell_deadmines_ride_magma_vehicle_SpellScript : public SpellScript
        {
            ...
            void HandleHit(SpellEffIndex effIndex)
            {
                if (Unit* caster = GetCaster())
                    GetHitUnit()->CastSpell(GetHitUnit(), GetSpellInfo()->GetEffect(effIndex)->BasePoints);
            }
    ...
    public:
        ...
        class spell_deadmines_bloodbath_AuraScript : public AuraScript
        {
            ...
            void HandleTick(AuraEffect const* /*aurEff*/)
            {
                ...
                if (Unit* caster = GetCaster())
                    if (caster->GetVictim())
                        caster->CastSpell(GetCaster()->GetVictim(), GetSpellInfo()->GetEffect(EFFECT_1)->TriggerSpell, true);
            }
