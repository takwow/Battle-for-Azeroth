...
public:
    ...
    class spell_q14098_knocking_67869_SpellScript : public SpellScript
    {
        ...
        bool Validate(SpellInfo const* spellInfo) override
        {
            SpellEffectInfo const* spellEffectInfo = spellInfo->GetEffect(RAND(0, 1) == 0 ? EFFECT_1 : EFFECT_2);
        ...
        void HandleEffect(SpellEffIndex /*effIndex*/)
        {
            if (SpellInfo const* spellInfo = GetSpellInfo())
                GetCaster()->CastSpell(GetCaster(), GetSpellInfo()->GetEffect(RAND(0, 1) == 0 ? EFFECT_1 : EFFECT_2)->BasePoints, true);
        }
